...
local function CreateWelcomeFrame()
    ...
    logo:SetTexture("Interface\\AddOns\\PaladinTools\\paladintools")
...
local steps = {
    {
        title = "The HUD",
        desc = "This is your HUD \226\128\148 it shows your reagent counts (Symbol of Kings) at a glance.",
        setup = function()
    ...
    },
    {
        ...
        desc = "This panel helps you track reagents and group size when buffing up before pulls.",
        setup = function()
            local bm = PT.modules["BuffManager"]
            if bm then
                bm:UpdateSessionProgress()
                PaladinToolsBlessingSession:Show()
            end
            return PaladinToolsBlessingSession
        end,
        teardown = function()
            if PaladinToolsBlessingSession and PaladinToolsBlessingSession:IsShown() then
                PaladinToolsBlessingSession:Hide()
            end
        end,
    },
    {
...
function Tour:Init()
    ...
    if not PaladinToolsDB.tourVersion or PaladinToolsDB.tourVersion < TOUR_VERSION then
        PaladinToolsDB.showSessionOnLogin = false
