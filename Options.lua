...
local ACCENT_COLOR = { 0.96, 0.55, 0.73 }
...
local function CreateCheckbox(parent, label, dbKey, yOffset, onChange)
    ...
    cb:SetChecked(PaladinToolsDB[dbKey])
    cb:SetScript("OnClick", function(self)
        local checked = not not self:GetChecked()
        PaladinToolsDB[dbKey] = checked
...
local function CreateSlider(parent, label, dbKey, minVal, maxVal, step, yOffset, onChange)
    ...
    local currentVal = PaladinToolsDB[dbKey]
...
local function CreateKeybind(parent, label, dbKey, yOffset)
    ...
    local function UpdateLabel()
        local key = PaladinToolsDB[dbKey]
...
local function CreateKeywordEditor(parent, yOffset)
    ...
    local function RefreshKeywords()
        ...
        local keywords = PaladinToolsDB.whisperKeywords
        for i, kw in ipairs(keywords) do
            ...
            row.removeBtn:SetScript("OnClick", function()
                tremove(PaladinToolsDB.whisperKeywords, i)
                RefreshKeywords()
            end)
            row:Show()
        end
    ...
    local function RepositionAdd()
        local count = #PaladinToolsDB.whisperKeywords
        local addY = -(count * 24)
    ...
    local function DoAddKeyword()
        ...
        if newKW ~= "" then
            tinsert(PaladinToolsDB.whisperKeywords, newKW)
            addBox:SetText("")
            addBox:ClearFocus()
            RefreshKeywords()
            RepositionAdd()
    ...
    local totalHeight = (#PaladinToolsDB.whisperKeywords * 24) + 30
...
local function BuildGeneralContent(parent)
    ...
    y = CreateCheckbox(parent, "Show HUD", "hudVisible", y, function(checked)
        ...
        if bm then
            if checked then
                if PaladinToolsHUD then PaladinToolsHUD:Show() end
            else
                if PaladinToolsHUD then PaladinToolsHUD:Hide() end
            end
        end
    end)
    ...
    for _, item in ipairs(categoryItems) do
        ...
        cb:SetChecked(PaladinToolsDB.popupCategories[catKey])
        cb:SetScript("OnClick", function(self)
            PaladinToolsDB.popupCategories[catKey] = not not self:GetChecked()
            local pm = PT.modules["PopupMenu"]
...
local function BuildBuffContent(parent)
    local y = -8
...
local function BuildAppearanceContent(parent)
    ...
    y = CreateSlider(parent, "Max Queue Display", "maxQueueDisplay", 5, 20, 1, y, function()
        local th = PT.modules["TradeHelper"]
...
local function CreateOptionsFrame()
    local f = CreateFrame("Frame", "PaladinToolsOptions", UIParent, "BackdropTemplate")
    ...
    tinsert(UISpecialFrames, "PaladinToolsOptions")

    local title = f:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
    title:SetPoint("TOP", 0, -10)
    title:SetText("|cffF58CBAPaladinTools Options|r")
...
function OPT:RegisterBlizzardOptions()
    ...
    panel:SetScript("OnShow", function(self)
        ...
        title:SetText("|cffF58CBAPaladinTools|r")
